...
# function to get assistance by month
def get_assistance_by_month(combinations, filepath, name_add, ie):
...
def process_single_realization_NoInf(real, filepath, ie):
    list_rows_by_date = []
    dict_dates = {}
    ...
    # get filtered dates
    df_filter = process_monthly_with_dates_filter(filepath, [real, 0, 100, 1.0, 'Baseline'], name_add, ie)

    # get household data
    df_hh = pd.read_parquet(
        filepath + 'df_assisted_bill_{}P{}T{}_dCV{}_real{}_demand{}_IE{}.parquet'.format(name_add, 100, 0, 1.0, real,
                                                                                    'Baseline', ie), columns=columns)
...
def process_single_real_inf(real, filepath, dict_accts_by_date, dP_All, dT_All, dCV, ie):
    list_rows_by_date = []
    dict_dates = {}
    ## get households under scenario with no inf
    columns = ['Date', 'account', 'totalWaterCosts', 'AR', 'does_acct_get_assistance?', 'totalWaterCostsAssist_income',
    ...
    for dT in dT_All:
        for dP in dP_All:
            ...
            df_hh = pd.read_parquet(
                ...
                columns=columns)
            # get filtered dates
            df_filter = process_monthly_with_dates_filter(filepath, [real, dT, dP, dCV, 'Baseline'], name_add, ie)
...
for real in real_All:
    ...
    df_monthly, dict_accts_lists = process_single_realization_NoInf(real, filepath, ie)

    # moderate scenario: get assistance amounts under adaptation conditions
    df_monthly_moderate = process_single_real_inf(real, filepath, dict_accts_lists, [100], [0, 1], 1.0, ie)

    # dry scenario: get assistance amounts under adaptation conditions
    df_monthly_dry = process_single_real_inf(real, filepath, dict_accts_lists, [80, 90], [4, 5], 1.2, ie)
...
df_monthly_baseline_All.to_csv(filepath + 'df_monthly_baseline_Fig4_IE.csv', index=False)
df_monthly_moderate_All.to_csv(filepath + 'df_monthly_moderate_Fig4_IE.csv', index=False)
df_monthly_dry_All.to_csv(filepath + 'df_monthly_dry_Fig4_IE.csv', index=False)

#%% import data once the above is run
df_monthly_baseline_All = pd.read_csv(filepath + 'df_monthly_baseline_Fig4_IE.csv')
df_monthly_moderate_All = pd.read_csv(filepath + 'df_monthly_moderate_Fig4_IE.csv')
df_monthly_dry_All = pd.read_csv(filepath + 'df_monthly_dry_Fig4_IE.csv')

#%% put together data for bar plot
# average
...
#%%  create bar plot for avg, max, min
...
plt.savefig('../../outputs/Figures/Figure4_IE_AssistDrivers.png', dpi=300, bbox_inches='tight')

#%% print statistics
# get percent of costs due to same households- moderate
print('percent of costs due to same HHs: moderate')
same_households = (avg_combined[1,0]+avg_combined[1,1])/(avg_combined[1,0]+avg_combined[1,1]+avg_combined[1,2])
print(same_households*100)

same_households = (max_combined[1,0]+max_combined[1,1])/(max_combined[1,0]+max_combined[1,1]+max_combined[1,2])
print(same_households*100)

same_households = (min_combined[1,0]+min_combined[1,1])/(min_combined[1,0]+min_combined[1,1]+min_combined[1,2])
print(same_households*100)

#%% get percent of costs due to same households- dry
print('percent of costs due to same HHs: dry')
same_households = (avg_combined[2,0]+avg_combined[2,1])/(avg_combined[2,0]+avg_combined[2,1]+avg_combined[2,2])
print(same_households*100)

same_households = (max_combined[2,0]+max_combined[2,1])/(max_combined[2,0]+max_combined[2,1]+max_combined[2,2])
print(same_households*100)

same_households = (min_combined[2,0]+min_combined[2,1])/(min_combined[2,0]+min_combined[2,1]+min_combined[2,2])
print(same_households*100)

#%% get percent of costs due to NEW households- moderate
print('percent of costs from NEW HHs: moderate')
new_households = (avg_combined[1,2])/(avg_combined[1,0]+avg_combined[1,1]+avg_combined[1,2])
print(new_households*100)

same_households = (max_combined[1,2])/(max_combined[1,0]+max_combined[1,1]+max_combined[1,2])
print(same_households*100)

same_households = (min_combined[1,2])/(min_combined[1,0]+min_combined[1,1]+min_combined[1,2])
print(same_households*100)

#%% get percent of costs due to NEW households- dry
print('percent of costs from NEW HHs: moderate')
new_households = (avg_combined[2,2])/(avg_combined[2,0]+avg_combined[2,1]+avg_combined[2,2])
print(new_households*100)

same_households = (max_combined[2,2])/(max_combined[2,0]+max_combined[2,1]+max_combined[2,2])
print(same_households*100)

same_households = (min_combined[2,2])/(min_combined[2,0]+min_combined[2,1]+min_combined[2,2])
print(same_households*100)
